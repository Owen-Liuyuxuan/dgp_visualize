...
class Pose2tfNode:
    def __init__(self):
        ...
        rospy.Subscriber("/dgp/CAM_FRONT/pose", PoseStamped, self.FRONT_pose_cb)
        rospy.Subscriber("/dgp/CAM_FRONT_LEFT/pose", PoseStamped, self.FRONT_LEFT_pose_cb)
        rospy.Subscriber("/dgp/CAM_FRONT_RIGHT/pose", PoseStamped, self.FRONT_RIGHT_pose_cb)
        rospy.Subscriber("/dgp/CAM_BACK/pose", PoseStamped, self.BACK_pose_cb)
        rospy.Subscriber("/dgp/CAM_BACK_LEFT/pose", PoseStamped, self.BACK_LEFT_pose_cb)
        rospy.Subscriber("/dgp/CAM_BACK_RIGHT/pose", PoseStamped, self.BACK_RIGHT_pose_cb)
        rospy.Subscriber("/dgp/LIDAR_TOP/pose", PoseStamped, self.LIDAR_TOP_pose_cb)
        rospy.Subscriber("/dgp/ego_pose", PoseStamped, self.ego_pose_cb)

        rospy.Subscriber("/dgp/control/index", Int32, self.index_callback) # to clean up initial pose
